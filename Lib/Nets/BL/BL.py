...
Description: This file implements the Baseline
"""
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
...
class BL:
    """
    This class implements all the variables, functions, methods, required to deploy a shallow U-Net which is used a
    classifier on top of a pretrained model
    """
    def __init__(self, opt, device):
        ...
        if self.opt.mode == "train":
            ...
            self.G_net_chain = itertools.chain(self.netG_S2O.parameters(), self.SegNet.parameters())
            self.optimizer = torch.optim.Adam(self.G_net_chain, lr=self.opt.lr, betas=(self.opt.beta1, 0.999))
    ...
    def forward(self):
        ...
        feature_map = self.netG_S2O(self.real_S)
    ...
    def load(self, file_GAN, file_SN):
        """
        ...
        if self.opt.mode == 'train':
            ...
            self.optimizer.load_state_dict(data_SN['opt'])
        elif self.opt.mode == 'eval':
            self.netG_S2O.load_state_dict(data_GAN['G_S2O'])
            self.SegNet.load_state_dict(data_SN['SN'])
    ...
    def tb_add_step_images(self, writer=None, global_step=None):
        ...
        if writer is None:
            ...
            writer.add_images("1 - Labes", label_norm, global_step=global_step)
            writer.add_images("2 - Map", seg_map_norm, global_step=global_step)
            writer.add_images("3 - Radar Input", real_S_norm, global_step=global_step)
    ...
    def train(self, train_dataset, eval_dataset, writer):
        ...
        """
        global_step = 0
        if self.opt.acc_log_freq == 0:
            calculate_accuracy(self, eval_dataset, writer, global_step, "Test", 0, self.posx_test, self.posy_test, self.opt.test_size, True)
            calculate_accuracy(self, train_dataset, writer, global_step, "Train", 0, self.posx_train, self.posy_train, self.opt.train_size, True)
        ...
        for epoch in range(self.opt.tot_epochs):
            t = start()
            text_line = "=" * 27 + "BL EPOCH " + str(epoch) + "/" + str(self.opt.tot_epochs) + "=" * 27
            print(text_line)
            ...
            if epoch >= 0 and epoch % self.opt.acc_log_freq == 0:
                calculate_accuracy(self, eval_dataset, writer, global_step, "Test", epoch, self.posx_test, self.posy_test, self.opt.test_size, True)
                calculate_accuracy(self, train_dataset, writer, global_step, "Train", epoch, self.posx_train, self.posy_train, self.opt.train_size, True)
                self.Logger_train.append_acc_step({"step": global_step})
                self.Logger_test.append_acc_step({"step": global_step})
